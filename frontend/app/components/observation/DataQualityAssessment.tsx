...
export default function DataQualityAssessment({
  ...
  const currentIdentifications = identifications.filter(
    (i) => i.current && !i.withdrawn,
  );

  const uniqueUsersWithSpecies = new Set(
    currentIdentifications
      .filter((i) => i.taxonRank === "SPECIES")
      .map((i) => i.userId),
  );

  const hasSpeciesLevelId = observation.communityTaxon?.rank === "SPECIES";
  const hasMultipleSpeciesIds =
    hasSpeciesLevelId && uniqueUsersWithSpecies.size >= 2;
  ...
  if (hasDisagreement) {
    checks.push({
      label: "Community consensus on species level ID",
      criteria: false,
    });
  }
      ...
      <div className="space-y-2">
        {checks.map((check, index) => (
          <div key={index} className="flex items-center gap-3 text-sm">
            ...
            <span
              className={check.criteria ? "text-stone-700" : "text-stone-400"}
            >
              {check.label}
            </span>
          </div>
      ...
      {!isResearchGrade && (
        <div className="mt-4 pt-4 border-t border-stone-100 bg-amber-50 rounded-lg p-3 flex items-start gap-2">
          ...
          <p className="text-xs text-amber-800 leading-relaxed">
            This observation needs more identifications to reach{" "}
            <strong>Research Grade</strong>. Suggest an identification to help!
          </p>
        </div>
      )}
    </div>
