# ============================================================
# EXP04: INCEPTION V3 — COMBINED BEST SETTINGS
# Combines the best findings from all previous experiments:
#   EXP01: label_smoothing = 0.1
#   EXP02: enhanced augmentation pipeline
#   EXP03: freeze_epochs   = 11
# ============================================================
...
class Config:
    ...
    SAVE_DIR  = r"C:\Users\Doruk\OneDrive\Masaüstü\Graduation Project\Bird Dataset Processed\model-final"
    ...
    # ── EXP01 best: label smoothing = 0.1 ────────────────────
    LABEL_SMOOTHING = 0.1
...
print("\n" + "=" * 70)
print("CONFIGURATION — EXP04 (COMBINED BEST SETTINGS)")
print("=" * 70)
print(f"  Model:              {Config.CURRENT_MODEL.upper()}")
print(f"  Label Smoothing:    {Config.LABEL_SMOOTHING}")
print(f"  Augmentation:       ENHANCED")
print(f"  Freeze epochs:      {Config.FREEZE_EPOCHS}← EXP03 best")
...
# ============================================================
# DATA TRANSFORMS
# EXP02 enhanced augmentation pipeline (inception_v3 → 299×299)
# ============================================================
...
def get_transforms():
    ...
    # ── EXP02 best: richer / stronger training augmentation ──
    ...
    # Val / test transforms are always the standard centre-crop pipeline
...
def get_model(num_classes, pretrained=True):
    ...
    # Freeze all layers; only the classifier heads are trainable in Phase 1
...
def train_phase(model, train_loader, val_loader, criterion,
    ...
    print(f"\n{'=' * 70}")
    print(f"  {phase_name}: INCEPTION_V3 (EXP04 — Combined)")
    print(f"{'=' * 70}")
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_pbar:
            ...
            # inception_v3 returns (outputs, aux_outputs) during training
        ...
        print(f"    Head LR: {current_lr:.6f} | Backbone LR: {backbone_lr_val:.6f} | "
              f"Time: {epoch_time / 60:.1f}m | Total: {elapsed / 60:.1f}m")
...
def plot_training_history(history, save_dir):
    ...
    axes[0].set_title('INCEPTION_V3 (EXP04 Combined) — Loss', fontweight='bold')
    axes[0].legend()
    ...
    axes[1].set_title('INCEPTION_V3 (EXP04 Combined) — Accuracy', fontweight='bold')
    axes[1].legend()
    axes[1].grid(True, alpha=0.3)

    plt.tight_layout()
    save_path = os.path.join(save_dir, 'inception_v3_combined_training_history.png')
...
def plot_confusion_matrix(labels, predictions, class_names, save_dir):
    ...
    plt.title('Confusion Matrix — INCEPTION_V3 (EXP04 Combined)',
    ...
    plt.tight_layout()
    save_path = os.path.join(save_dir, 'inception_v3_combined_confusion_matrix.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print("\n" + "#" * 70)
    print("#  EXP04 — INCEPTION V3 COMBINED BEST SETTINGS")
    print("#  EXP01: label_smoothing=0.1")
    print("#  EXP02: enhanced augmentation")
    print("#  EXP03: freeze_epochs=11")
    print("#" * 70)
    ...
    test_dataset  = BirdDataset(Config.TEST_CSV,  Config.IMAGE_PATH_COL,
                                Config.LABEL_COL, transform=val_transform)
    ...
    # ── EXP01 best: label_smoothing = 0.1 ────────────────────
    ...
    print("\n" + "=" * 70)
    print(f"  PHASE 1: TRAINING CLASSIFIER HEAD (backbone frozen, {Config.FREEZE_EPOCHS} epochs)")
    print("=" * 70)

    head_params       = [p for p in model.parameters() if p.requires_grad]
    optimizer_phase1  = optim.Adam(head_params, lr=Config.HEAD_LR,
                                   weight_decay=Config.WEIGHT_DECAY)
    scheduler_phase1  = optim.lr_scheduler.CosineAnnealingLR(
        optimizer_phase1, T_max=Config.FREEZE_EPOCHS,
    ...
    # Save final model
    final_path = os.path.join(Config.SAVE_DIR, 'inception_v3_combined_final.pth')
    torch.save({
        'model_state_dict': model.state_dict(),
        'best_acc':         best_acc,
        'history':          history,
        ...
        'augmentation':     'enhanced_exp02'
    }, final_path)

    print(f"\n  Training complete for INCEPTION_V3 (EXP04 Combined)")
    ...
    print("\n" + "=" * 70)
    print("  FINAL RESULTS: INCEPTION_V3 (EXP04 — Combined Best Settings)")
    print("=" * 70)
    ...
    results_csv_path = os.path.join(Config.SAVE_DIR, 'exp04_results.csv')
