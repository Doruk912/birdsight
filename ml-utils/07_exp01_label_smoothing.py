# ============================================================
# EXP01: INCEPTION V3 WITH LABEL SMOOTHING
# Based on 04_train_models.py — inception_v3 only
# Change: criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
# ============================================================
...
class Config:
    ...
    SAVE_DIR = r"C:\Users\Doruk\OneDrive\Masaüstü\Graduation Project\Bird Dataset Processed\models\exp01_label_smoothing"
...
# ============================================================
# DATA TRANSFORMS  (inception_v3 uses 299x299)
# ============================================================
...
def train_phase(model, train_loader, val_loader, criterion,
    ...
    print(f"\n{'=' * 70}")
    print(f"  {phase_name}: INCEPTION_V3")
    print(f"{'=' * 70}")
...
def plot_training_history(history, save_dir):
    ...
    axes[0].set_title('INCEPTION_V3 (Label Smoothing) - Loss', fontweight='bold')
    axes[0].legend()
    ...
    axes[1].set_title('INCEPTION_V3 (Label Smoothing) - Accuracy', fontweight='bold')
    axes[1].legend()
    axes[1].grid(True, alpha=0.3)

    plt.tight_layout()
    save_path = os.path.join(save_dir, 'inception_v3_label_smoothing_training_history.png')
...
def plot_confusion_matrix(labels, predictions, class_names, save_dir):
    ...
    plt.title('Confusion Matrix - INCEPTION_V3 (Label Smoothing)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    save_path = os.path.join(save_dir, 'inception_v3_label_smoothing_confusion_matrix.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print("\n" + "#" * 70)
    print(f"#  EXP01 - INCEPTION V3 WITH LABEL SMOOTHING ({Config.LABEL_SMOOTHING})")
    print("#" * 70)
    ...
    # Save final model
    final_path = os.path.join(Config.SAVE_DIR, 'inception_v3_label_smoothing_final.pth')
    torch.save({
        'model_state_dict': model.state_dict(),
        'best_acc': best_acc,
        'history': history,
    ...
    print(f"\n  Training complete for INCEPTION_V3 (label smoothing={Config.LABEL_SMOOTHING})")
    ...
    print("\n" + "=" * 70)
    print("  FINAL RESULTS: INCEPTION_V3 (Label Smoothing)")
    print("=" * 70)
    ...
    # Save results
    results = {
        'experiment': 'exp01_label_smoothing',
        'model': model_name,
        'label_smoothing': Config.LABEL_SMOOTHING,
    ...
    results_csv_path = os.path.join(Config.SAVE_DIR, 'exp01_results.csv')
